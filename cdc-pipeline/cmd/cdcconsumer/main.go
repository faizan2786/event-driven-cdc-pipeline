...
func main() {
	...
	if err != nil {
		fmt.Printf("failed to init cassandra client: %v\n", err)
		os.Exit(1)
	}
	defer cs.Close()
	...
	for {
		...
		if err != nil {
			...
			if err == context.DeadlineExceeded {
				fmt.Printf("No new messages received in last %d seconds for topic %s. Shutting down consumer...\n", timeOutSecs, topic)
				break
			}
		...
		if err != nil {
			fmt.Printf("message parsing error: topic=%s partition=%d offset=%d: %v\n", msg.Topic, msg.Partition, msg.Offset, err)
			break
		}

		// apply to Cassandra (idempotent due to processed_events table)
		if err := cqlClient.ApplyChange(topic, ev); err != nil {
			fmt.Printf("apply change error: topic=%s partition=%d offset=%d: %v\n", msg.Topic, msg.Partition, msg.Offset, err)
			break
		}
