...
func (rq *realQuery) Scan(dest ...interface{}) error {
	return rq.q.Scan(dest)
}
...
func (c *CassandraClient) applyUserChange(ev *model.ChangeEvent) error {
	...
	case "u":
		modifiedAt := row["modified_at"].(string)
		isDeleted, _ := row["is_deleted"].(bool)

		// if it's a deleting update, just set the deleted_ind (without updating the name)
		if isDeleted {
			stmt := "UPDATE users SET modified_at = ?, is_deleted = ? WHERE id = ?"
			return c.session.Query(stmt, modifiedAt, true, id).Exec()
		} else {
			...
			err := c.session.Query(readStmt, id).Scan(&currName)
			if err != nil {
				return fmt.Errorf("applyUserChange: error when reading existing record with user id %v\n%w", id, err)
			}
			...
			if err != nil {
				return fmt.Errorf("applyUserChange: error when inserting updated record for user id %s\n%w", id, err)
			}
...
func (c *CassandraClient) applyOrderChange(ev *model.ChangeEvent) error {
	...
	total := parser.ParseDebeziumDecimal(row["total_amount"].(string), 2)

	switch ev.Op {
	case "c":
		// the timestamps are RFC3339 formatted string so just pass as is to cassandra
		placedAt := row["placed_at"].(string)
	...
	case "u":
		...
		q1 := `UPDATE orders SET status = ?, modified_at = ?, is_deleted = ? WHERE order_id = ?`
		if err := c.session.Query(q1, status, modifiedAt, isDeleted, orderID).Exec(); err != nil {
			return err
		}
...
func convertJsonValueToCqlUUID(idVal any) (gocql.UUID, error) {
	...
	if err != nil {
		return gocql.UUID{}, fmt.Errorf("applyUserChange: invalid UUID format\n%w", err)
	}
	return uuid, nil
}
